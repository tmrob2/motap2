...
pub fn witness(hullset: &Vec<Vec<f64>>, target: &Vec<f64>, dim: &usize, num_agents: &usize) -> Vec<f64> {
    ...
    for i in 0..*dim {
        ...
        let count_contrib = q_transpose[i].iter().fold(0u32, |sum, &val| if val > 0.0 { sum + 1u32} else { sum });
        //println!("data constirbutions for var {}: {}", i, count_contrib);
        if q_sum >0.0 && count_contrib >= *num_agents as u32 {
