...
impl TeamMDP {
    ...
    pub fn multi_obj_sched_synth<'a>(&self, target: &Vec<f64>, eps: &f64, rewards: &Rewards, state_index_map: &'a [usize],
                                     transition_map: &'a [usize], state_to_trans_cardinality: &'a [usize],
                                     state_to_trans_start_fin_map: &'a [(usize,usize)], task_agent_map: &'a [(usize,usize)],
                                     sprime_state_index_map: &'a [usize], task_agent_sprime_map: &'a [usize])
        -> Alg1Output {
    ...
    pub fn multi_obj_sched_synth_non_iter<'a>(&self, target: &Vec<f64>, eps: &f64, rewards: &Rewards) -> Alg1Output {
        let mut hullset: Vec<Vec<f64>> = Vec::new();
        let mut mu_vect: Vec<Vec<String>> = Vec::new();
        let mut alg1_output: Alg1Output = Alg1Output{
            v: vec![],
